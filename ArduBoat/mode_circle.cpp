...
bool ModeCircle::_enter()
{
    ...
    // record center as location (only used for reporting)
    config.center_loc = boat.current_loc;
...
void ModeCircle::update_drive_to_radius()
{
    ...
    g2.pos_control.input_pos_target(target.pos, boat.G_Dt);
}
...
void ModeCircle::update_circling()
{
    ...
    const float speed_change_max = (g2.pos_control.get_accel_max() * 0.5 * boat.G_Dt);
    ...
    if (!tracking_back) {
        ...
        const float angle_dt = angular_rate_rad * boat.G_Dt;
    ...
    target.accel = Vector2f(-sq(target.speed) / config.radius, accel_fb / boat.G_Dt);
