...
// called at 10Hz
void Boat::crash_check()
{
    static uint16_t crash_counter;  // number of iterations vehicle may have been crashed
    bool crashed = false; // stores crash state

    // return immediately if disarmed, crash checking is disabled or vehicle is Hold, Manual or Acro mode (if it is not a balance boat)
    if (!arming.is_armed() \
        || g.fs_crash_check == FS_CRASH_DISABLE \
        || ((!control_mode->is_autopilot_mode()) && (!is_balancebot()))) {
        crash_counter = 0;
        return;
    }
    ...
    if ((g2.crash_angle != 0) \
        && ((fabsf(ahrs.get_pitch()) > radians(g2.crash_angle)) \
            || (fabsf(ahrs.get_roll())  > radians(g2.crash_angle)))) {
        crashed = true;
    }

    if (!is_balancebot()) {
        if (!crashed && (
            (ahrs.groundspeed()      >= CRASH_CHECK_VEL_MIN) ||  // Check velocity
        ...
        // check if crashing for threshold seconds
