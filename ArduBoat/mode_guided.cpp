        ...
        case SubMode::WP:
        {
            ...
            if (!g2.wp_nav.reached_destination()) {
                ...
                if (send_notification) {
                    send_notification = false;
                    boat.gcs().send_mission_item_reached_message(0);
                }
                ...
                _distance_to_destination = boat.current_loc.get_distance(g2.wp_nav.get_destination());
            }
            break;
        }
...
bool ModeGuided::set_desired_location(const Location &destination, Location next_destination)
{
    ...
    boat.Log_Write_GuidedTarget((uint8_t)_guided_mode, Vector3f(destination.lat, destination.lng, 0), Vector3f(g2.wp_nav.get_speed_max(), 0.0f, 0.0f));
#endif
    return true;
}
...
#if HAL_LOGGING_ENABLED
    // log new target
    boat.Log_Write_GuidedTarget((uint8_t)_guided_mode, Vector3f(_desired_yaw_cd, 0.0f, 0.0f), Vector3f(_desired_speed, 0.0f, 0.0f));
#endif
}
...
#if HAL_LOGGING_ENABLED
    // log new target
    boat.Log_Write_GuidedTarget((uint8_t)_guided_mode, Vector3f(_desired_yaw_rate_cds, 0.0f, 0.0f), Vector3f(_desired_speed, 0.0f, 0.0f));
#endif
}
...
void ModeGuided::limit_init_time_and_location()
{
    ...
    limit.start_loc = boat.current_loc;
}
...
bool ModeGuided::limit_breached() const
{
    ...
    if (is_positive(limit.horiz_max)) {
        return (boat.current_loc.get_distance(limit.start_loc) > limit.horiz_max);
    }
