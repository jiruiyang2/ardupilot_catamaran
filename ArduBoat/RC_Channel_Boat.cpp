...
{
    return boat.g.mode_channel;
}

void RC_Channel_Rover::mode_switch_changed(modeswitch_pos_t new_pos)
{
    if (new_pos < 0 || (uint8_t)new_pos > boat.num_modes) {
        // should not have been called
        return;
    }

    boat.set_mode((Mode::Number)boat.modes[new_pos].get(), ModeReason::RC_COMMAND);
}
...
{
    return boat.failsafe.bits & FAILSAFE_EVENT_THROTTLE;
}
        ...
        const AuxSwitchPos ch_flag)
{
    ...
    case AuxSwitchPos::HIGH:
        boat.set_mode(mode, ModeReason::AUX_FUNCTION);
        break;
    case AuxSwitchPos::MIDDLE:
        // do nothing
        break;
    case AuxSwitchPos::LOW:
        if (boat.control_mode == &mode) {
...
void RC_Channel_Rover::add_waypoint_for_current_loc()
{
    ...
    cmd.content.location = boat.current_loc;

    // make the new command to a waypoint
    cmd.id = MAV_CMD_NAV_WAYPOINT;

    // save command
    if (boat.mode_auto.mission.add_cmd(cmd)) {
        hal.console->printf("Added waypoint %u", (unsigned)boat.mode_auto.mission.num_commands());
    }
}
...
void RC_Channel_Rover::do_aux_function_sailboat_motor_3pos(const AuxSwitchPos ch_flag)
{
    switch (ch_flag) {
    case AuxSwitchPos::HIGH:
        boat.g2.sailboat.set_motor_state(Sailboat::UseMotor::USE_MOTOR_ALWAYS);
        break;
    case AuxSwitchPos::MIDDLE:
        boat.g2.sailboat.set_motor_state(Sailboat::UseMotor::USE_MOTOR_ASSIST);
        break;
    case AuxSwitchPos::LOW:
        boat.g2.sailboat.set_motor_state(Sailboat::UseMotor::USE_MOTOR_NEVER);
        break;
    }
}
...
bool RC_Channel_Rover::do_aux_function(const AuxFuncTrigger &trigger)
{
    ...
    case AUX_FUNC::SAVE_WP:
        if (ch_flag == AuxSwitchPos::HIGH) {
            ...
            if (boat.control_mode == &boat.mode_auto) {
                break;
            }

            // if disarmed clear mission and set home to current location
            if (!boat.arming.is_armed()) {
                boat.mode_auto.mission.clear();
    ...
    case AUX_FUNC::LEARN_CRUISE:
        if (ch_flag == AuxSwitchPos::HIGH) {
            boat.cruise_learn_start();
        }
        break;

    // set mode to Manual
    case AUX_FUNC::MANUAL:
        do_aux_function_change_mode(boat.mode_manual, ch_flag);
        break;

    // set mode to Acro
    case AUX_FUNC::ACRO:
        do_aux_function_change_mode(boat.mode_acro, ch_flag);
        break;

    // set mode to Steering
    case AUX_FUNC::STEERING:
        do_aux_function_change_mode(boat.mode_steering, ch_flag);
        break;

    // set mode to Hold
    case AUX_FUNC::HOLD:
        do_aux_function_change_mode(boat.mode_hold, ch_flag);
        break;

    // set mode to Auto
    case AUX_FUNC::AUTO:
        do_aux_function_change_mode(boat.mode_auto, ch_flag);
        break;

    // set mode to RTL
    case AUX_FUNC::RTL:
        do_aux_function_change_mode(boat.mode_rtl, ch_flag);
        break;

    // set mode to SmartRTL
    case AUX_FUNC::SMART_RTL:
        do_aux_function_change_mode(boat.mode_smartrtl, ch_flag);
        break;

    // set mode to Guided
    case AUX_FUNC::GUIDED:
        do_aux_function_change_mode(boat.mode_guided, ch_flag);
        break;

    // Set mode to LOITER
    case AUX_FUNC::LOITER:
        do_aux_function_change_mode(boat.mode_loiter, ch_flag);
        break;

#if MODE_FOLLOW_ENABLED
    // Set mode to Follow
    case AUX_FUNC::FOLLOW:
        do_aux_function_change_mode(boat.mode_follow, ch_flag);
        break;
#endif

    // set mode to Simple
    case AUX_FUNC::SIMPLE:
        do_aux_function_change_mode(boat.mode_simple, ch_flag);
        break;

    case AUX_FUNC::CIRCLE:
        do_aux_function_change_mode(boat.g2.mode_circle, ch_flag);
        break;
    ...
    // save steering trim
    case AUX_FUNC::TRIM_TO_CURRENT_SERVO_RC:
        if (!boat.g2.motors.have_skid_steering() && boat.arming.is_armed() &&
            (boat.control_mode != &boat.mode_loiter)
            && (boat.control_mode != &boat.mode_hold) && ch_flag == AuxSwitchPos::HIGH) {
