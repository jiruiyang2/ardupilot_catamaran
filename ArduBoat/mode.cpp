...
Mode::Mode() :
    ...
    g(boat.g),
    g2(boat.g2),
    channel_steer(boat.channel_steer),
    channel_throttle(boat.channel_throttle),
    channel_lateral(boat.channel_lateral),
    channel_roll(boat.channel_roll),
    channel_pitch(boat.channel_pitch),
    channel_walking_height(boat.channel_walking_height),
...
bool Mode::enter()
{
    ...
    if (!ignore_checks) {
        ...
        boat.ahrs.get_filter_status(filt_status);

        // check position estimate.  requires origin and at least one horizontal position flag to be true
        const bool position_ok = boat.ekf_position_ok() && !boat.failsafe.ekf;
...
void Mode::get_pilot_input(float &steering_out, float &throttle_out) const
{
    ...
    if (boat.failsafe.bits & FAILSAFE_EVENT_THROTTLE) {
        steering_out = 0;
        throttle_out = 0;
        return;
    }
    ...
    switch ((PilotSteerType)g.pilot_steer_type.get())
    {
        ...
        default: {
            ...
            throttle_out = boat.channel_throttle->get_control_in();
            const float steering_dir = is_negative(throttle_out) ? -1 : 1;
            steering_out = steering_dir * boat.channel_steer->get_control_in();
            break;
        }
        ...
        case PilotSteerType::TWO_PADDLES: {
            ...
            const float left_paddle = boat.channel_steer->norm_input_dz();
            const float right_paddle = boat.channel_throttle->norm_input_dz();
        ...
        case PilotSteerType::DIR_UNCHANGED_WHEN_REVERSING: {
            throttle_out = boat.channel_throttle->get_control_in();
            steering_out = boat.channel_steer->get_control_in();
            break;
        }
    }
}
...
void Mode::get_pilot_desired_steering_and_throttle(float &steering_out, float &throttle_out) const
{
    ...
    float throttle_out_limited = g2.motors.get_slew_limited_throttle(throttle_out, boat.G_Dt);
...
void Mode::get_pilot_desired_steering_and_speed(float &steering_out, float &speed_out) const
{
    ...
    // check for special case of input and output throttle being in opposite directions
    float speed_out_limited = g2.attitude_control.get_desired_speed_accel_limited(speed_out, boat.G_Dt);
...
void Mode::get_pilot_desired_lateral(float &lateral_out) const
{
    ...
    if ((boat.failsafe.bits & FAILSAFE_EVENT_THROTTLE) || (boat.channel_lateral == nullptr)) {
        lateral_out = 0;
        return;
    }

    // get pilot lateral input
    lateral_out = boat.channel_lateral->get_control_in();
}
...
void Mode::get_pilot_desired_heading_and_speed(float &heading_out, float &speed_out) const
{
    ...
    float desired_steering = constrain_float(boat.channel_steer->norm_input_dz(), -1.0f, 1.0f);
    float desired_throttle = constrain_float(boat.channel_throttle->norm_input_dz(), -1.0f, 1.0f);
...
void Mode::calc_throttle(float target_speed, bool avoidance_enabled)
{
    ...
    target_speed = attitude_control.get_desired_speed_accel_limited(target_speed, boat.G_Dt);

#if AP_AVOIDANCE_ENABLED
    // apply object avoidance to desired speed using half vehicle's maximum deceleration
    if (avoidance_enabled) {
        g2.avoid.adjust_speed(0.0f, 0.5f * attitude_control.get_decel_max(), ahrs.get_yaw(), target_speed, boat.G_Dt);
        if (g2.sailboat.tack_enabled() && g2.avoid.limits_active()) {
            // we are a sailboat trying to avoid fence, try a tack
            if (boat.control_mode != &boat.mode_acro) {
                boat.control_mode->handle_tack_request();
            }
        }
    }
    ...
    if (g2.sailboat.sail_enabled()) {
        ...
        if (is_zero(target_speed) && !boat.is_balancebot()) {
            bool stopped;
            throttle_out = 100.0f * attitude_control.get_throttle_out_stop(g2.motors.limit.throttle_lower, g2.motors.limit.throttle_upper, g.speed_cruise, g.throttle_cruise * 0.01f, boat.G_Dt, stopped);
        } else {
            ...
            throttle_out = 100.0f * attitude_control.get_throttle_out_speed(target_speed, motor_lim_low, motor_lim_high, g.speed_cruise, g.throttle_cruise * 0.01f, boat.G_Dt);
        }

        // if vehicle is balance bot, calculate actual throttle required for balancing
        if (boat.is_balancebot()) {
            boat.balancebot_pitch_control(throttle_out);
        }
    }
...
bool Mode::stop_vehicle()
{
    ...
    if (boat.is_balancebot()) {
        throttle_out = 100.0f * attitude_control.get_throttle_out_speed(0, g2.motors.limit.throttle_lower, g2.motors.limit.throttle_upper, g.speed_cruise, g.throttle_cruise * 0.01f, boat.G_Dt);
        boat.balancebot_pitch_control(throttle_out);
    } else {
        throttle_out = 100.0f * attitude_control.get_throttle_out_stop(g2.motors.limit.throttle_lower, g2.motors.limit.throttle_upper, g.speed_cruise, g.throttle_cruise * 0.01f, boat.G_Dt, stopped);
    }
    ...
    if (!stopped) {
        steering_out = attitude_control.get_steering_out_rate(0.0, g2.motors.limit.steer_left, g2.motors.limit.steer_right, boat.G_Dt);
    }
...
Mode *Boat::mode_from_mode_num(const enum Mode::Number num)
{
    Mode *ret = nullptr;
    switch (num) {
