...
void Sailboat::init()
{
    // sailboat defaults
    if (sail_enabled()) {
        boat.g2.crash_angle.set_default(0);
        ...
        boat.g2.wp_nav.enable_overspeed(motor_state != UseMotor::USE_MOTOR_ALWAYS);
    }

    if (tack_enabled()) {
        boat.g2.loit_type.set_default(1);
    }
...
void Sailboat::init_rc_in()
{
    ...
    if (rc_ptr != nullptr) {
        ...
        channel_mainsail = boat.channel_throttle;
    }
}
...
void Sailboat::set_pilot_desired_mainsail()
{
    ...
    if ((boat.failsafe.bits & FAILSAFE_EVENT_THROTTLE) || (channel_mainsail == nullptr)) {
       relax_sails();
    } else {
       boat.g2.motors.set_mainsail(constrain_float(channel_mainsail->get_control_in(), 0.0f, 100.0f));
       boat.g2.motors.set_wingsail(constrain_float(channel_mainsail->get_control_in(), -100.0f, 100.0f));
       boat.g2.motors.set_mast_rotation(constrain_float(channel_mainsail->get_control_in(), -100.0f, 100.0f));
    }
}
...
void Sailboat::set_auto_mainsail(float desired_speed)
{
    ...
    const float pid_offset = boat.g2.attitude_control.get_sail_out_from_heel(radians(sail_heel_angle_max), boat.G_Dt) * 100.0f;

    // get apparent wind, + is wind over starboard side, - is wind over port side
    const float wind_dir_apparent = degrees(boat.g2.windvane.get_apparent_wind_direction_rad());
    ...
    boat.g2.motors.set_mainsail(mainsail_out);
    //
    // wingsail control
    ...
    boat.g2.motors.set_wingsail(wingsail_out);
    //
    ...
    boat.g2.motors.set_mast_rotation(mast_rotation_out);
}

void Sailboat::relax_sails()
{
    boat.g2.motors.set_mainsail(100.0f);
    boat.g2.motors.set_wingsail(0.0f);
    boat.g2.motors.set_mast_rotation(0.0f);
}
...
void Sailboat::get_throttle_and_set_mainsail(float desired_speed, float &throttle_out)
{
    ...
    if ((motor_state == UseMotor::USE_MOTOR_ALWAYS) ||
        ...
        throttle_out = 100.0f * boat.g2.attitude_control.get_throttle_out_speed(desired_speed,
                                                                        boat.g2.motors.limit.throttle_lower,
                                                                        boat.g2.motors.limit.throttle_upper,
                                                                        boat.g.speed_cruise,
                                                                        boat.g.throttle_cruise * 0.01f,
                                                                        boat.G_Dt);
    }
...
float Sailboat::get_VMG() const
{
    ...
    float speed;
    if (!boat.g2.attitude_control.get_forward_speed(speed)) {
        return 0.0f;
    }
    ...
    return (speed * cosf(wrap_PI(radians(boat.g2.wp_nav.wp_bearing_cd() * 0.01f) - boat.ahrs.get_yaw())));
}
...
void Sailboat::handle_tack_request_acro()
{
    ...
    tack_heading_rad = wrap_2PI(boat.ahrs.get_yaw() + 2.0f * wrap_PI((boat.g2.windvane.get_true_wind_direction_rad() - boat.ahrs.get_yaw())));
...
{
    if (fabsf(wrap_PI(tack_heading_rad - boat.ahrs.get_yaw())) < radians(SAILBOAT_TACKING_ACCURACY_DEG) ||
...
bool Sailboat::use_indirect_route(float desired_heading_cd) const
{
    ...
    return fabsf(wrap_PI(boat.g2.windvane.get_true_wind_direction_rad() - desired_heading_rad)) <= radians(sail_no_go + SAILBOAT_NOGO_PAD);
}
...
float Sailboat::calc_heading(float desired_heading_cd)
{
    ...
    const AP_WindVane::Sailboat_Tack current_tack = boat.g2.windvane.get_current_tack();
    ...
    const float true_wind_rad = boat.g2.windvane.get_true_wind_direction_rad();
    if (fabsf(wrap_PI(true_wind_rad - desired_heading_rad)) > radians(sail_no_go) && !currently_tacking) {
        ...
        if (new_tack != current_tack) {
            ...
            const float app_wind_rad = boat.g2.windvane.get_apparent_wind_direction_rad();
    ...
    const float cross_track_error = boat.g2.wp_nav.crosstrack_error();
...
void Sailboat::set_motor_state(UseMotor state, bool report_failure)
{
    ...
    if (boat.g2.motors.have_skid_steering() ||
        ...
        boat.get_frame_type() != boat.g2.motors.frame_type::FRAME_TYPE_UNDEFINED) {
        motor_state = state;
...
bool Sailboat::motor_assist_low_wind() const
{
    ...
    return (is_positive(sail_windspeed_min) &&
            boat.g2.windvane.wind_speed_enabled() &&
            (boat.g2.windvane.get_true_wind_speed() < sail_windspeed_min));
}
