...
void ModeAuto::update()
{
    ...
    switch (_submode) {
        case SubMode::WP:
        {
            ...
            if (boat.is_boat() && g2.wp_nav.reached_destination() && !g2.wp_nav.is_fast_waypoint()) {
        ...
        case SubMode::RTL:
            boat.mode_rtl.update();
            break;

        case SubMode::Loiter:
            boat.mode_loiter.update();
            break;
...
float ModeAuto::wp_bearing() const
{
    ...
    case SubMode::Stop:
        return 0.0f;
    case SubMode::RTL:
        return boat.mode_rtl.wp_bearing();
    case SubMode::Loiter:
        return boat.mode_loiter.wp_bearing();
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.wp_bearing();
    case SubMode::Circle:
...
float ModeAuto::nav_bearing() const
{
    ...
    case SubMode::Stop:
        return 0.0f;
    case SubMode::RTL:
        return boat.mode_rtl.nav_bearing();
    case SubMode::Loiter:
        return boat.mode_loiter.nav_bearing();
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.nav_bearing();
    case SubMode::Circle:
...
float ModeAuto::crosstrack_error() const
{
    ...
    case SubMode::Stop:
        return 0.0f;
    case SubMode::RTL:
        return boat.mode_rtl.crosstrack_error();
    case SubMode::Loiter:
        return boat.mode_loiter.crosstrack_error();
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.crosstrack_error();
    case SubMode::Circle:
...
float ModeAuto::get_desired_lat_accel() const
{
    ...
    case SubMode::Stop:
        return 0.0f;
    case SubMode::RTL:
        return boat.mode_rtl.get_desired_lat_accel();
    case SubMode::Loiter:
        return boat.mode_loiter.get_desired_lat_accel();
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.get_desired_lat_accel();
    case SubMode::Circle:
...
float ModeAuto::get_distance_to_destination() const
{
    ...
    case SubMode::Stop:
        ...
        return 0.0f;
    case SubMode::RTL:
        return boat.mode_rtl.get_distance_to_destination();
    case SubMode::Loiter:
        return boat.mode_loiter.get_distance_to_destination();
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.get_distance_to_destination();
    case SubMode::Circle:
...
bool ModeAuto::get_desired_location(Location& destination) const
{
    ...
    case SubMode::Stop:
        ...
        return false;
    case SubMode::RTL:
        return boat.mode_rtl.get_desired_location(destination);
    case SubMode::Loiter:
        return boat.mode_loiter.get_desired_location(destination);
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.get_desired_location(destination);
    case SubMode::Circle:
...
bool ModeAuto::reached_destination() const
{
    ...
    case SubMode::Stop:
        ...
        break;
    case SubMode::RTL:
        return boat.mode_rtl.reached_destination();
        break;
    case SubMode::Loiter:
        return boat.mode_loiter.reached_destination();
        break;
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.reached_destination();
    case SubMode::Circle:
...
bool ModeAuto::set_desired_speed(float speed)
{
    ...
    case SubMode::HeadingAndSpeed:
        ...
        return true;
    case SubMode::RTL:
        return boat.mode_rtl.set_desired_speed(speed);
    case SubMode::Loiter:
        return boat.mode_loiter.set_desired_speed(speed);
    case SubMode::Guided:
    case SubMode::NavScriptTime:
        return boat.mode_guided.set_desired_speed(speed);
    case SubMode::Circle:
...
bool ModeAuto::check_trigger(void)
{
    ...
    if (auto_triggered && g.auto_trigger_pin != -1 && boat.check_digital_pin(g.auto_trigger_pin) == 1) {
    ...
    if (g.auto_trigger_pin != -1 && boat.check_digital_pin(g.auto_trigger_pin) == 0) {
    ...
    if (!is_zero(g.auto_kickstart)) {
        const float xaccel = boat.ins.get_accel().x;
...
{
    if (boat.mode_guided.enter()) {
        _submode = SubMode::Guided;

        // initialise guided start time and position as reference for limit checking
        boat.mode_guided.limit_init_time_and_location();

        // sanity check target location
        if ((loc.lat != 0) || (loc.lng != 0)) {
            guided_target.loc = loc;
            guided_target.loc.sanitize(boat.current_loc);
...
bool ModeAuto::start_command(const AP_Mission::Mission_Command& cmd)
{
    ...
    case MAV_CMD_DO_SET_ROI:
        if (cmd.content.location.alt == 0 && cmd.content.location.lat == 0 && cmd.content.location.lng == 0) {
            ...
            if (boat.camera_mount.get_mode() == MAV_MOUNT_MODE_GPS_POINT) {
                boat.camera_mount.set_mode_to_default();
            }
        } else {
            // send the command to the camera mount
            boat.camera_mount.set_roi_target(cmd.content.location);
        }
        break;
#endif
...
void ModeAuto::exit_mission()
{
    ...
    case DoneBehaviour::ACRO:
        if (boat.set_mode(boat.mode_acro, ModeReason::MISSION_END)) {
            return;
        }
        break;
    case DoneBehaviour::MANUAL:
        if (boat.set_mode(boat.mode_manual, ModeReason::MISSION_END)) {
            return;
        }
        break;
    }
...
bool ModeAuto::do_nav_wp(const AP_Mission::Mission_Command& cmd, bool always_stop_at_destination)
{
    ...
    cmdloc.sanitize(boat.current_loc);
...
bool ModeAuto::verify_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (guided_target.valid) {
        if (boat.current_loc.get_distance(guided_target.loc) <= g2.wp_nav.get_radius()) {
            return true;
        }
    }

    // guided command complete once a limit is breached
    return boat.mode_guided.limit_breached();
}
...
bool ModeAuto::do_circle(const AP_Mission::Mission_Command& cmd)
{
    ...
    circle_center.sanitize(boat.current_loc);
...
{
    if (cmd.p1 == 1 && boat.have_position) {
        if (!boat.set_home_to_current_location(false)) {
            // ignored...
        }
    } else {
        if (!boat.set_home(cmd.content.location, false)) {
            // ignored...
        }
    }
}
