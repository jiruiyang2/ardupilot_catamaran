...
bool ModeDock::_enter()
{
    ...
    if (!boat.precland.enabled() || !boat.precland.target_acquired()) {
    ...
    g2.pos_control.set_reversed(boat.precland.get_orient() == 4);
...
void ModeDock::update()
{
    ...
    const bool real_dock_in_sight = boat.precland.get_target_position_cm(_dock_pos_rel_origin_cm);
    ...
    if (_distance_to_destination <= stopping_dist || (force_real_target && !real_dock_in_sight)) {
        ...
        if (boat.is_boat()) {
            ...
            _loitering = boat.mode_loiter.enter();
        }
        return;
    }
    ...
    g2.pos_control.input_pos_target(target_pos, boat.G_Dt);
    g2.pos_control.update(boat.G_Dt);
