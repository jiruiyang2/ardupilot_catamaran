...
{
    if (boat.control_mode != &boat.mode_simple) {
        return false;
    }
    return (boat.g2.simple_type == ModeSimple::Simple_InitialHeading);
}
...
bool GCS_Rover::supersimple_input_active() const
{
    ...
    return (boat.g2.simple_type == ModeSimple::Simple_CardinalDirections);
}
...
void GCS_Rover::update_vehicle_sensor_status_flags(void)
{
    ...
    if (boat.control_mode->attitude_stabilized()) {
