...
uint8_t GCS_MAVLINK_Rover::base_mode() const
{
    ...
    if (boat.control_mode->has_manual_input()) {
    ...
    if (boat.g2.stick_mixing > 0 && boat.control_mode != &boat.mode_initializing) {
    ...
    if (boat.control_mode != &boat.mode_initializing && boat.arming.is_armed()) {
...
{
    return (uint32_t)boat.control_mode->mode_number();
}

MAV_STATE GCS_MAVLINK_Rover::vehicle_system_status() const
{
    if ((boat.failsafe.triggered != 0) || boat.failsafe.ekf) {
        return MAV_STATE_CRITICAL;
    }
    if (boat.control_mode == &boat.mode_initializing) {
        return MAV_STATE_CALIBRATING;
    }
    if (boat.control_mode == &boat.mode_hold) {
...
{
    Location target;
    if (!boat.control_mode->get_desired_location(target)) {
        return;
    }
...
void GCS_MAVLINK_Rover::send_nav_controller_output() const
{
    ...
    const Mode *control_mode = boat.control_mode;

    mavlink_msg_nav_controller_output_send(
        chan,
        0,  // roll
        degrees(boat.g2.attitude_control.get_desired_pitch()),
...
{
    float motor1, motor3;
    if (boat.g2.motors.have_skid_steering()) {
...
{
    return boat.g2.motors.get_throttle();
}
...
void GCS_MAVLINK_Rover::send_rangefinder() const
{
    ...
    for (uint8_t i=0; i<boat.rangefinder.num_sensors(); i++) {
        AP_RangeFinder_Backend *s = boat.rangefinder.get_backend(i);
        if (s == nullptr) {
            continue;
        }
        if (!got_one ||
...
void Boat::send_wheel_encoder_distance(const mavlink_channel_t chan)
{
...
{
    return boat.control_mode != &boat.mode_initializing;
}
...
bool GCS_MAVLINK_Rover::try_send_message(enum ap_message id)
{
    ...
    case MSG_WHEEL_DISTANCE:
        ...
        boat.send_wheel_encoder_distance(chan);
        break;

    case MSG_WIND:
        CHECK_PAYLOAD_SIZE(WIND);
        boat.g2.windvane.send_wind(chan);
        break;
...
{
    if (!boat.control_mode->in_guided_mode()) {
        // only accept position updates when in GUIDED mode
        return false;
    }

    // make any new wp uploaded instant (in case we are already in Guided mode)
    return boat.mode_guided.set_desired_location(cmd.content.location);
}
...
{
    if (packet.y == 1) {
        if (boat.g2.windvane.start_direction_calibration()) {
            return MAV_RESULT_ACCEPTED;
        } else {
            return MAV_RESULT_FAILED;
        }
    } else if (packet.y == 2) {
        if (boat.g2.windvane.start_speed_calibration()) {
            return MAV_RESULT_ACCEPTED;
        } else {
            return MAV_RESULT_FAILED;
        }
...
MAV_RESULT GCS_MAVLINK_Rover::handle_command_int_packet(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
    ...
    case MAV_CMD_DO_CHANGE_SPEED:
        ...
        if (!boat.control_mode->set_desired_speed(packet.param2)) {
            return MAV_RESULT_FAILED;
        }
        return MAV_RESULT_ACCEPTED;
    ...
    case MAV_CMD_DO_SET_REVERSE:
        ...
        boat.control_mode->set_reversed(is_equal(packet.param1,1.0f));
        return MAV_RESULT_ACCEPTED;

    case MAV_CMD_NAV_RETURN_TO_LAUNCH:
        if (boat.set_mode(boat.mode_rtl, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_ACCEPTED;
        }
        return MAV_RESULT_FAILED;
    ...
    case MAV_CMD_DO_MOTOR_TEST:
        ...
        return boat.mavlink_motor_test_start(*this,
    ...
    case MAV_CMD_MISSION_START:
        ...
        if (boat.set_mode(boat.mode_auto, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_ACCEPTED;
        }
        return MAV_RESULT_FAILED;
...
MAV_RESULT GCS_MAVLINK_Rover::handle_command_nav_set_yaw_speed(const mavlink_command_int_t &packet, const mavlink_message_t &msg)
{
        ...
        if (packet.param3 > 0) {
            // relative angle
            boat.mode_guided.set_desired_heading_delta_and_speed(packet.param1 * 100.0f, packet.param2);
        } else {
            // absolute angle
            boat.mode_guided.set_desired_heading_and_speed(packet.param1 * 100.0f, packet.param2);
        }
        return MAV_RESULT_ACCEPTED;
}
#endif
...
MAV_RESULT GCS_MAVLINK_Rover::handle_command_int_do_reposition(const mavlink_command_int_t &packet)
{
    ...
    if (!boat.control_mode->in_guided_mode() && !change_modes) {
        return MAV_RESULT_DENIED;
    }
    ...
    if (!boat.control_mode->in_guided_mode()) {
        if (!boat.set_mode(Mode::Number::GUIDED, ModeReason::GCS_COMMAND)) {
            return MAV_RESULT_FAILED;
        }
    }

    if (is_positive(packet.param1)) {
        if (!boat.control_mode->set_desired_speed(packet.param1)) {
            return MAV_RESULT_FAILED;
        }
    }

    // set the destination
    if (!boat.mode_guided.set_desired_location(requested_location)) {
        return MAV_RESULT_FAILED;
    }
...
{
    manual_override(boat.channel_steer, packet.y, 1000, 2000, tnow);
    manual_override(boat.channel_throttle, packet.z, 1000, 2000, tnow);
}
...
void GCS_MAVLINK_Rover::handle_set_attitude_target(const mavlink_message_t &msg)
{
    ...
    const float target_speed = boat.control_mode->get_speed_default() * packet.thrust;
    ...
    if ((packet.type_mask & MAVLINK_SET_ATT_TYPE_MASK_YAW_RATE_IGNORE) != 0) {
        ...
        boat.mode_guided.set_desired_heading_and_speed(target_heading_cd, target_speed);
    } else {
        // use body_yaw_rate field
        boat.mode_guided.set_desired_turn_rate_and_speed((RAD_TO_DEG * packet.body_yaw_rate) * 100.0f, target_speed);
    }
}
...
void GCS_MAVLINK_Rover::handle_set_position_target_local_ned(const mavlink_message_t &msg)
{
    ...
    // need ekf origin
    Location ekf_origin;
    if (!boat.ahrs.get_origin(ekf_origin)) {
        return;
    }
    ...
    Location target_loc = boat.current_loc;
    if (!pos_ignore) {
        switch (packet.coordinate_frame) {
        case MAV_FRAME_BODY_NED:
        case MAV_FRAME_BODY_OFFSET_NED: {
            // rotate from body-frame to NE frame
            const float ne_x = packet.x * boat.ahrs.cos_yaw() - packet.y * boat.ahrs.sin_yaw();
            const float ne_y = packet.x * boat.ahrs.sin_yaw() + packet.y * boat.ahrs.cos_yaw();
    ...
    if (!vel_ignore) {
        const float speed_max = boat.control_mode->get_speed_default();
        ...
        if (packet.coordinate_frame == MAV_FRAME_BODY_NED || packet.coordinate_frame == MAV_FRAME_BODY_OFFSET_NED) {
            target_yaw_cd = wrap_180_cd(target_yaw_cd + boat.ahrs.yaw_sensor);
        }
    }
    ...
    // by default, we consider that the boat will drive forward
    ...
    if (!pos_ignore) {
        ...
        if (!boat.mode_guided.set_desired_location(target_loc)) {
    ...
    if (!vel_ignore && yaw_ignore && yaw_rate_ignore) {
        // consume velocity
        boat.mode_guided.set_desired_heading_and_speed(target_yaw_cd, speed_dir * target_speed);
    } else if (!vel_ignore && yaw_ignore && !yaw_rate_ignore) {
        // consume velocity and turn rate
        boat.mode_guided.set_desired_turn_rate_and_speed(target_turn_rate_cds, speed_dir * target_speed);
    } else if (!vel_ignore && !yaw_ignore && yaw_rate_ignore) {
        // consume velocity and heading
        boat.mode_guided.set_desired_heading_and_speed(target_yaw_cd, speed_dir * target_speed);
    } else if (vel_ignore && !yaw_ignore && yaw_rate_ignore) {
        // consume just target heading (probably only skid steering vehicles can do this)
        boat.mode_guided.set_desired_heading_and_speed(target_yaw_cd, 0.0f);
    } else if (vel_ignore && yaw_ignore && !yaw_rate_ignore) {
        // consume just turn rate (probably only skid steering vehicles can do this)
        boat.mode_guided.set_desired_turn_rate_and_speed(target_turn_rate_cds, 0.0f);
    }
}
...
void GCS_MAVLINK_Rover::handle_landing_target(const mavlink_landing_target_t &packet, uint32_t timestamp_ms)
{
    ...
    boat.precland.handle_msg(packet, timestamp_ms);
#endif
}
...
{
    if (boat.g2.windvane.enabled()) {
        // return units are m/s*5
        return MIN(boat.g2.windvane.get_true_wind_speed() * 5, UINT8_MAX);
    }
    return 0;
}
...
uint8_t GCS_MAVLINK_Rover::high_latency_wind_direction() const
{
    if (boat.g2.windvane.enabled()) {
        ...
        return wrap_360(degrees(boat.g2.windvane.get_true_wind_direction_rad())) / 2;
    }
    return 0;
}
...
uint8_t GCS_MAVLINK_Rover::send_available_mode(uint8_t index) const
{
    const Mode* modes[] {
        ...
        &boat.mode_smartrtl,
        &boat.mode_guided,
        &boat.mode_initializing,
#if MODE_DOCK_ENABLED
        (Mode *)boat.g2.mode_dock_ptr,
#endif
    };
