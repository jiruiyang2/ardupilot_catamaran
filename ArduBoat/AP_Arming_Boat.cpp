...
{
    if (!require_location &&
        !boat.control_mode->requires_position() &&
        !boat.control_mode->requires_velocity()) {
        // we don't care!
        return true;
    }
...
bool AP_Arming_Rover::pre_arm_checks(bool report)
{
    ...
    if (boat.g2.sailboat.sail_enabled() && !boat.g2.windvane.enabled()) {
...
bool AP_Arming_Rover::arm(AP_Arming::Method method, const bool do_arming_checks)
{
    ...
    boat.g2.smart_rtl.set_home(true);

    // initialize simple mode heading
    boat.mode_simple.init_heading();

    // save home heading for use in sail vehicles
    boat.g2.windvane.record_home_heading();
...
{
    char failure_msg[50] = {};
    if (boat.g2.oa.pre_arm_check(failure_msg, ARRAY_SIZE(failure_msg))) {
        return true;
    }
...
bool AP_Arming_Rover::parameter_checks(bool report)
{
    ...
    if (!is_positive(boat.g2.wp_nav.get_default_speed())) {
...
bool AP_Arming_Rover::mode_checks(bool report)
{
    ...
    if (!boat.control_mode->allows_arming()) {
...
{
    bool ret = boat.g2.motors.pre_arm_check(report);
